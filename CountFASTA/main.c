...
void help( void ) {
	fprintf( stdout, "\n" );
	fprintf( stdout, "Program: Count line and size of FASTA files.\n" );
	fprintf( stdout, "Author: Akira Hasegawa (ah3q@gsc.riken.jp)\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Usage: CountFASTA INPUT [INPUT2 ..] > OUTPUT\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "       INPUT   Input FASTA file\n" );
	fprintf( stdout, "       OUTPUT  Line count and file size of FASTA files.\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Output: test/input.fa	96 B\n" );
	fprintf( stdout, "        test/input.fa	8 line\n" );
	fprintf( stdout, "        test/input.fa	4 sequence\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Note: This program is used by MOIRAI to count sequences in FASTA file.\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Updates: Unified help message across perl, java, and shell scripts.\n" );
	fprintf( stdout, "         2011/10/31  Outputs statistical counts of FASTA file.\n" );
	fprintf( stdout, "\n" );
}
