...
void help( void ) {
    fprintf( stdout, "\n" );
	fprintf( stdout, "Program: Extract sequence line from FASTA/FASTQ/SAM/BAM input file.\n" );
	fprintf( stdout, "Author: Akira Hasegawa (ah3q@gsc.riken.jp)\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Usage: simpleSeq < INPUT > OUTPUT\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "       INPUT   Input FASTA/FASTQ/SAM/BAM format file\n" );
	fprintf( stdout, "       OUTPUT  Text with sequence lines only\n" );
	fprintf( stdout, "\n" );
	...
	fprintf( stdout, "         2011/11/30  Pick only sequences.\n" );
	fprintf( stdout, "\n" );
	exit( 0 );
}
...
int main ( int argc, char* argv[] ) {
	...
	while( hasMoreStringVector( files ) ) {
		...
		while( readNextSequence( reader ) ) fprintf( writer, "%s\n", reader->currentSequence );
