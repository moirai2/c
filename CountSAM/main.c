...
void help( void ) {
	fprintf( stdout, "\n" );
	fprintf( stdout, "Program: Count line and size of SAM files.\n" );
	fprintf( stdout, "Author: Akira Hasegawa (ah3q@gsc.riken.jp)\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Usage: CountSAM INPUT [INPUT2 ..] > OUTPUT\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "       INPUT   Input SAM file\n" );
	fprintf( stdout, "       OUTPUT  Line count and file size of SAM files.\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Output: test/input.sam	2102975 B\n" );
	fprintf( stdout, "        test/input.sam	7249 line\n" );
	fprintf( stdout, "        test/input.sam	27 head line\n" );
	fprintf( stdout, "        test/input.sam	7222 SAM line\n" );
	fprintf( stdout, "        test/input.sam	6920 primary alignment\n" );
	fprintf( stdout, "        test/input.sam	302 secondary alignment\n" );
	fprintf( stdout, "        test/input.sam	0 unmapped sequence\n" );
	fprintf( stdout, "        test/input.sam	3207 positive strand\n" );
	fprintf( stdout, "        test/input.sam	4015 negative strand\n" );
	fprintf( stdout, "        test/input.sam	6920 sequence\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Output(paired-end): input.sam   373224586 B\n" );
	fprintf( stdout, "                    input.sam   10205566 line\n" );
	fprintf( stdout, "                    input.sam   0 head line\n" );
	fprintf( stdout, "                    input.sam   6195295 sequence line\n" );
	fprintf( stdout, "                    input.sam   981206 primary alignment\n" );
	fprintf( stdout, "                    input.sam   147097 secondary alignment\n" );
	fprintf( stdout, "                    input.sam   0 unmapped sequence\n" );
	fprintf( stdout, "                    input.sam   3327744 positive strand\n" );
	fprintf( stdout, "                    input.sam   2867551 negative strand\n" );
	fprintf( stdout, "                    input.sam   1128303 properly aligned\n" );
	fprintf( stdout, "                    input.sam   5066992 inadequately aligned\n" );
	fprintf( stdout, "                    input.sam   4010271 read2 sequence line\n" );
	fprintf( stdout, "                    input.sam   981206 read2 primary alignment\n" );
	fprintf( stdout, "                    input.sam   147097 read2 secondary alignment\n" );
	fprintf( stdout, "                    input.sam   0 read2 unmapped sequence\n" );
	fprintf( stdout, "                    input.sam   1901701 read2 positive strand\n" );
	fprintf( stdout, "                    input.sam   2108570 read2 negative strand\n" );
	fprintf( stdout, "                    input.sam   1128303 read2 properly aligned\n" );
	fprintf( stdout, "                    input.sam   2881968 read2 inadequately aligned\n" );
	fprintf( stdout, "                    input.sam   981206 sequence\n" );
	fprintf( stdout, "                    input.sam   981206 read2 sequence\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Note: This program is used by MOIRAI to count sequences in SAM file.\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Updates: 2013/05/14  'sequence' added to enable MOIRAI to track numbers.\n" );
	fprintf( stdout, "         2011/11/22  Outputs statistical counts of SAM file.\n" );
	fprintf( stdout, "\n" );
}
...
int main ( int argc, char* argv[] ) {
	...
	for( int i = 1; i < argc; i++ ) {
		...
		if( paired_flag == 1 ) { // paired-end
			...
			fprintf( stdout, "%s\t%ld read2 SAM line\n", argv[ i ], sequenceCount2  );
