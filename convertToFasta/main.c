...
void help( void ) {
    fprintf( stdout, "\n" );
	fprintf( stdout, "Program: Convert BAM/SAM/FASTQ to FASTA format.\n" );
	fprintf( stdout, "Author: Akira Hasegawa (ah3q@gsc.riken.jp)\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Usage: convertToFasta INPUT > OUTPUT\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "       INPUT  FASTQ/FASTA/BAM/SAM file(s)\n" );
	fprintf( stdout, "       OUTPUT FASTA format file\n" );
	fprintf( stdout, "\n" );
	fprintf( stdout, "Options: -h  Show help menu\n" );
	fprintf( stdout, "         -O  FASTA format file (default=STDOUT)\n" );
	fprintf( stdout, "\n" );
	...
	fprintf( stdout, "         2012/10/26  Print outs FASTA lines.\n" );
	fprintf( stdout, "\n" );
	exit( 0 );
}
...
int main ( int argc, char* argv[] ) {
	...
	while( hasMoreStringVector( files ) ) {
		...
		while( readNextSequence( reader ) ) printCurrentFASTA( reader, writer );
